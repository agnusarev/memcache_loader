...
def insert_appsinstalled(
    memc_addr: Any, appsinstalled: Any, dry_run: bool = False
) -> bool:
...
def parse_appsinstalled(line: Any) -> Union[AppsInstalled, None]:
    ...
    try:
        ...
        apps = [int(a.strip()) for a in raw_apps.split(",") if a.isidigit()]  # type: ignore
...
def main(options: Any) -> None:
    ...
    for fn in glob.iglob(options.pattern):
        ...
        for line in fd:
            ...
            if not line:
                continue
            appsinstalled = parse_appsinstalled(line)  # type: ignore
